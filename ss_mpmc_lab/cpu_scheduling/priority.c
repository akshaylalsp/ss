...
struct process
{
    int pid,at,bt,wt,ta,pt;
}processes[10],temp;

void sort(int n){
    for(int i=0;i<n-1;i++){
        for(int j=0;j<n-1-i;j++){
            if(processes[j].pt > processes[j+1].pt ){
                temp = processes[j];
...
void display(int n){
    ...
    for(int i=0;i<n;i++){
        ...
        printf("\n%d\t%d\t%d\t%d\t%d",processes[i].pid,processes[i].pt,processes[i].bt,processes[i].wt,processes[i].ta);
    }
...
void read(int n){
    ...
    for(int i=0;i<n;i++){
        scanf("%d%d",&processes[i].pt,&processes[i].bt);
        processes[i].pid = i;
        processes[i].at = 0;
    }
}
